...
declaracion: NOMBRE_TIPO {eliminarNodosVariablesAdeclarar(&variablesAdeclarar);tipo=$<str>1;} punteroOpc IDENTIFICADOR arrayOpc {id = $<str>4; auxiliarFun.parametros = NULL;}bifurcacionDeclaraciones
;
...
declaracionVariable: inicializacionOpcional { insertarVariables(&variablesAdeclarar,tipo, id, linActual,cantPunteros,flagArray, $<str>1); cantPunteros = 0;flagArray=0;} op_RecursionListaDeDeclaradores ';' {declararVariablesProvisorias(variablesAdeclarar,&listaDeVars);printf("decaracion?:%s",$1);}
;
...
expPrimaria: IDENTIFICADOR {auxIdInvocacion = $1; char* tipo = buscarEnTabla(listaDeVars,$1); $$ = tipo; } 
    | NUM { $$ = "int" ;}
    | FLOAT {$$ = "float";}
...
declaracionFuncion:  '('{auxiliarFun.numLinea = linActual; auxiliarFun.tipo = tipo; auxiliarFun.identificador = id;} op_listaParametros ')'  division_decla_y_def ;
...
parametro: NOMBRE_TIPO IDENTIFICADOR {insertarUltimoParametro(&listaAuxParametrosDecla, $1, $2);}
